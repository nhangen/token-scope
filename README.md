...
| Codex | `~/.codex/sessions/**/*.jsonl` rollouts | Per-response input/output, cache read/write, and reasoning tokens attributed to the preceding model and effort | Billing route unknown (not in rollout records). OpenAI-style totals include cached/cache-write input and reasoning output; uncached input requires both cache classes, and visible output requires reasoning tokens. Missing classes stay unknown and are marked partial. Impossible totals are retained and marked malformed. Snapshot suppression requires a complete, valid cumulative tuple; incomplete cumulative snapshots are retained and marked partial. Cumulative-only prefixes and older rollouts emit a labeled aggregate with unknown response attribution. Event ids use a valid unique record ordinal, otherwise the line index, so appends stay stable and resumed files stay distinct. |
...
- Provider JSON rows include malformed, partial, and legacy-cumulative event counts. Any such observation makes top-level `measured` false and replaces the complete-measurement footer with an integrity warning.
...
- Reported numeric values come from source records rather than estimates; integrity fields identify unknown coverage or attribution. Row provenance (distinct source files) ships in the `--json` rows.

### Codex root-task experiments

`token-scope --codex-experiment manifest.json` compares persisted experiments by root thread ID. Each root includes only ancestry-consistent transitive subagents; blank root sources and subagents with missing or malformed ancestry stay wholly unknown and excluded. A missing root sets integrity to `missing-root`. Selected malformed, partial-class, or legacy aggregate observations set integrity to `incomplete-events`; text reports them as incomplete observations rather than complete responses. Either condition makes the report incomplete. Astra rows remain separate. `--json` preserves known disjoint token-class subtotals and reports versioned normalized weights as derived values only when class coverage and attribution are complete, never as measured subscription quota.

The version 1 manifest stores a `normalization` version and weights plus an `experiments` object keyed by root thread ID. Each experiment records `strategy`, `protocol.freshContext`, `protocol.forkTurns`, `protocol.acceptanceTest`, optional `acceptanceResult`, `elapsedSeconds`, `humanReworkMinutes`, optional `baselineEventIds`, and optional official account-meter `before`/`after` snapshots. Meter snapshots require a unit, `used` or `remaining` direction, valid timestamps, and an `after` timestamp later than `before`; reports show both signed change and direction-normalized consumption.
