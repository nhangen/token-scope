...
- **The rung ladder rescaled with the cap.** `RUNG_PCTS` moved from `[100, 50, 25, 10, 5]` to `[14, 7, 3.5, 1.4, 0.7]`, restoring the old absolute triggers (168M, 84M, 42M, 16.8M, 8.4M credits). Measured across 963 real sessions: the old ladder against 1.2B fired on 1/1/1/14 sessions (100%/50%/25%/5%; 10% unmeasured); the new ladder fires on 5/13/18/~37 (14%/7%/3.5%/0.7%; 1.4% unmeasured), matching the pre-rescale incidence. A rung that never fires is indistinguishable from "no session was heavy enough."
...
- **`--credits` JSON output: `partial` renamed to `open`.** The boolean field indicating whether a week is still in progress is now `open` in `--credits --json`, matching the window-side `CreditWindow` type. Consumers reading `.weeks[].partial` will get `undefined` (treated as `false` by most parsers). (#30)

### Added
- **Codex usage is attributed per response and can be compared by root-task experiment.** The adapter now binds each `last_token_usage` observation to its own model and effort, emits only disjoint token classes with known coverage, suppresses only complete and valid repeated cumulative snapshots, retains malformed and partial observations, preserves cumulative-only prefixes as labeled aggregates, preserves valid unique ordinal IDs with line fallback, and excludes missing or inconsistent ancestry. Provider and experiment reports propagate malformed, partial, and legacy integrity instead of claiming complete measurement or response attribution. `--codex-experiment <manifest.json>` flags missing roots, validates directed account-meter snapshots, separates Astra escalation, and labels normalized weights as derived rather than measured subscription quota. (#84)
- **`--savings` excludes runs an escalation superseded.** When a spec hit the turn cap twice, `ollama-delegate` now hands it to a higher-tier author instead of stopping, and that author is billed for the same job — so pricing the local attempts as "what Claude would have cost" counted one job twice. token-scope reads the sidecar `$XDG_STATE_HOME/ollama-agent/escalations.jsonl` (also `$OLLAMA_AGENT_ESCALATIONS`, or `--escalations <path>`) and drops a matching run from the counterfactual, keeping its tokens in the ledger totals and naming the exclusion in a footnote. A run matches on `superseded_run_id` **and** `cwd`, must itself have failed, and must fall in the `OLLAMA_ATTEMPT_GAP` window (default 4h) before the record. New per-row JSON keys, emitted only when something was superseded: `superseded_run_count`, `superseded_input`, `superseded_output`, `superseded_excluded_usd`. New top-level keys, emitted **always** beside `ledger_path`: `escalations_path`, `escalations_status` (`ok`/`absent`/`unreadable`), `escalations_records`, `escalations_skipped_lines`, `escalations_unmatched`, `attempt_gap_seconds` — a sidecar that failed to load and a sidecar that legitimately held nothing both produce zero exclusions, and without these the two are the same report. An unreadable sidecar, and a missing one whose path you named, warn on stderr; records that matched no run and lines that yielded no record are footnoted. (#81)

### Changed
- **The savings report discloses the ledger's own load status.** `readLedgerWithStatus` mirrors the escalations reader, and `--savings` emits `ledger_status` (`ok`/`absent`/`unreadable`) and `ledger_skipped_lines` beside `ledger_path`, warning on stderr when the file is there and cannot be read. `readLedger` keeps its old signature for the provider path. Before this, an unreadable ledger produced a confident, complete-looking zero — and shipping five fields about the sidecar's load status and none about the primary source would have advertised an integrity the report did not have. (#81 review)
- **Every flag taking a required value rejects a value beginning with `--`.** `--escalations --json` used to take `"--json"` as the path and swallow the flag; `--session --json` was the same defect and worse, because the six-character length check made it look validated. Now covers `--ledger`, `--escalations`, `--session`, `--db`, `--projects-dir`, `--agent`, `--pm-agent`, `--counterfactual-model`, and the three `--artifact-*` flags. (#81 review)
...
- **`--agent` and `--pm-agent` flags for per-agent subagent attribution.** `--agent <id>` scopes subagent spend to one agent; `--pm-agent` shows only the PM's own subagents. New: `querySubagentSpendByAgent()` and `SubagentSpend` per-agent breakdown. (#18)
- **Cross-file `message.id` deduplication pinned.** When the same message appears across multiple project dirs (worktree slug splits), the first file wins. This is the behavior production depends on; a future refactor can't flip it silently. (#21)
